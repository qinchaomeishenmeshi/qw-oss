...
import { TtOssOptions, TtOssOptionsWithFetch, TtOssOptionsWithApi } from './typings'
...
export class QwOss {
  // 单例
  private static globalTtOssInstance: QwOss | undefined
  // TtOss的配置
   ...
   */
  static upload(...args: Parameters<QwOss['upload']>) {
   ...
   * @return {string}
   */
  static getRelativePathFromUrl(...args: Parameters<typeof getRelativePathFromUrl>) {
   ...
   * @return {Promise<OssUploadResponse>}
   */
  upload(file: File, options?: (Parameters<typeof simpleUpload>[1] | Parameters<typeof multipartUpload>[1]) & { chunk?: boolean }) {
  ...
  ossApiInfo2OssClientOptions(ossApiInfo: OssApiInfo) {
    const { accessKeyId, accessKeySecret, regionId, bucketName, securityToken, expiration } = ossApiInfo
    return {
  ...
  private isTtOssOptionsWithFetch(options: TtOssOptions): options is TtOssOptionsWithFetch {
    return !!(options as TtOssOptionsWithFetch).fetchOssInfo
  }

  private isTtOssOptionsWithApi(options: TtOssOptions): options is TtOssOptionsWithApi {
